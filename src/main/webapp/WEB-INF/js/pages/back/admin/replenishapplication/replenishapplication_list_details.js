...
function addDetails(tdid) {
	trInfo = 	$("<tr id='dettr-"+tdid+"' class='text-danger'>" +
				...
				"	<td><input type='text' id='name-"+tdid+"' value='等待查询...' size='30'/></td>" +
				"	<td><input type='text' id='weight-"+tdid+"' value='0' size='8' /></td>" +
				"	<td><input type='text' id='amount-"+tdid+"' value='0' size='8' /></td>" +
				"	<td>" +
...
function saveDetails(did) {
	...
	operateAlert(true,"成品添加成功！","成品添加失败！") ;
...
function deleteDetails(did) {
	...
	// ajax移除信息，而后删除表格；
	$("#dettr-" + did).remove() ;
	operateAlert(true,"成品删除成功！","成品删除失败！") ;
}