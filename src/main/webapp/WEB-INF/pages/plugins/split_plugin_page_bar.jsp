...
<div id="splitControlDiv" style="float:right">
	<ul class="pagination"> 
		<li class="<%=currentPage == 1 ? "active" : ""%>"><a href="<%=handleUrl%>?cp=1&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&startTime=<%=startTime %>&endTime=<%=endTime %>">1</a></li>
<%
	if (allPages >= seed * 2 ) {	// 表示后面的页数已经超过了5页
		if (currentPage < seed * 2 - 1) {
			for (int x = 2 ; x < seed * 2 ; x ++) {
%>
			<li class="<%=currentPage == x ? "active" : ""%>"><a href="<%=handleUrl%>?cp=<%=x%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&startTime=<%=startTime %>&endTime=<%=endTime %>"><%=x%></a></li>
<%
			}
		} else {
			if (currentPage > seed * 2 - 1) {
...
<%
	if (allPages > 1) {
%>
		<li class="<%=currentPage == allPages ? "active" : ""%>"><a href="<%=handleUrl%>?cp=<%=allPages%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&startTime=<%=startTime %>&endTime=<%=endTime %>"><%=allPages%></a></li>
<%
	}
%>
	</ul><br>
